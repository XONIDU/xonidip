...
Funciona en: Windows, Linux (todas las distros), Mac
...
def get_install_flags():
    ...
    if sistema == 'linux':
        ...
        if distro in ['ubuntu', 'debian', 'mint']:
            # Debian/Ubuntu/Mint - usar --break-system-packages
            flags.append('--break-system-packages')
        elif distro in ['fedora', 'centos', 'rhel']:
            # Red Hat/Fedora/CentOS - usar --user
            flags.append('--user')
        elif distro in ['arch', 'manjaro']:
            # Arch/Manjaro - usar --break-system-packages
            flags.append('--break-system-packages')
        elif distro == 'opensuse':
            # OpenSUSE - usar --user
            flags.append('--user')
        else:
            # Linux genérico - preguntar al usuario
            flags.append('--break-system-packages')  # Por defecto
    ...
    if success:
        ...
        if sistema == 'linux':
            if distro in ['ubuntu', 'debian', 'mint', 'arch', 'manjaro']:
                print(f"   pip install -r requisitos.txt --break-system-packages")
            elif distro in ['fedora', 'centos', 'opensuse']:
                print(f"   pip install --user -r requisitos.txt")
            else:
                print(f"   pip install -r requisitos.txt --break-system-packages")
        elif sistema == 'darwin':
            print(f"   pip3 install -r requisitos.txt --user")
        else:
...
def get_install_instructions():
    """Obtiene instrucciones de instalación específicas para el sistema"""
    sistema = get_system()
    distro = get_linux_distro()
...
def main():
    ...
    if not os.path.exists('start.py'):
        ...
        print(f"   Asegurate de ejecutar este script en la carpeta correcta")
        input(f"\n{Colors.YELLOW}Presiona Enter para salir...{Colors.END}")
        return
...
def crear_accesos():
    ...
    if sistema == 'windows':
        ...
        with open('XONIDIP.bat', 'w') as f:
            f.write("""@echo off
title XONIDIP 2026
color 1F
python lanzador_universal.py
pause
""")
        print(f"{Colors.GREEN}Creado XONIDIP.bat - Haz doble clic para ejecutar{Colors.END}")
    
    elif sistema == 'linux':
        # Crear .sh para Linux
        with open('XONIDIP.sh', 'w') as f:
            f.write("""#!/bin/bash
python3 lanzador_universal.py
read -p "Presiona Enter para salir"
""")
        # Dar permisos de ejecución
        os.chmod('XONIDIP.sh', 0o755)
        print(f"{Colors.GREEN}Creado XONIDIP.sh - Ejecuta con: ./XONIDIP.sh{Colors.END}")
    
    elif sistema == 'darwin':
        # Crear .command para Mac
        with open('XONIDIP.command', 'w') as f:
            f.write("""#!/bin/bash
cd "$(dirname "$0")"
python3 lanzador_universal.py
""")
        os.chmod('XONIDIP.command', 0o755)
        print(f"{Colors.GREEN}Creado XONIDIP.command - Haz doble clic para ejecutar{Colors.END}")

def mostrar_info_instalacion():
    """Muestra información específica de instalación según la distro"""
    sistema = get_system()
    distro = get_linux_distro()
    
    print(f"\n{Colors.BOLD}Informacion para {sistema}{Colors.END}")
    
    if sistema == 'linux':
        print(f"{Colors.YELLOW}Comando de instalacion recomendado:{Colors.END}")
        if distro in ['ubuntu', 'debian', 'mint']:
            print("   pip install -r requisitos.txt --break-system-packages")
            print("\nSi tienes problemas, prueba:")
            print("   python3 -m pip install -r requisitos.txt --break-system-packages")
        elif distro in ['fedora', 'centos']:
            print("   pip install --user -r requisitos.txt")
            print("\nSi tienes problemas, prueba:")
            print("   python3 -m pip install --user -r requisitos.txt")
        elif distro in ['arch', 'manjaro']:
            print("   pip install -r requisitos.txt --break-system-packages")
            print("\nSi tienes problemas, prueba:")
            print("   python3 -m pip install -r requisitos.txt --break-system-packages")
